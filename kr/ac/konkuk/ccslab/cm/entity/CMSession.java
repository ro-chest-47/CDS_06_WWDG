...
public class CMSession extends CMSessionInfo {
	...
	// for our project
	public CMSession(String sname, String saddr, int port, int nUserNum, int nPossibleGroup) {
		super(sname, saddr, port, nUserNum, nPossibleGroup);
		m_groupList = new Vector<CMGroup>();
		m_sessionUsers = new CMMember();
		m_sessionConfFileName = null;
	}
	...
	// for our project
	public synchronized void countPossibleGroup() {
		...
		while(iter.hasNext()) {
			...
			tGroup = iter.next();
			if (tGroup.getGroupState()) {
				count++;
			}
		}
		
		super.m_nPossibleGroups = count;
	}
	// for our project
}
