...
public class CMInteractionManager {
	...
	private static void processREQUEST_SESSION_INFO(CMMessage msg, CMInfo cmInfo)
	{
		...
		while(iterSession.hasNext())
		{
			...
			// for our project
			tsession.countPossibleGroup();
			CMSessionInfo tInfo = new CMSessionInfo(tsession.getSessionName(), confInfo.getServerAddress(),
								  confInfo.getServerPort(), tsession.getSessionUsers().getMemberNum(), tsession.getPossibleGroups());
			// for our project	
	...
	private static void processRESPONSE_SESSION_INFO(CMMessage msg, CMInfo cmInfo)
	{
		...
		if(CMInfo._CM_DEBUG)
		{
			...
			// for our project
			System.out.format("%-20s%-20s%-10s%-10s%-10s%n", "name", "address", "port", "user num", "groups");
			// for our project
			System.out.format("%-60s%n", "------------------------------------------------------------");
		}
		...
		while(iter.hasNext())
		{
			...
			if(tSession == null)
			{
				...
				// for our project
				tSession = new CMSession(tInfo.getSessionName(), tInfo.getAddress(), tInfo.getPort(), 
						tInfo.getUserNum(), tInfo.getPossibleGroups());
				// for our project
			...
			else
			{
				...
				// for our project
				tSession.setPossibleGroups(tInfo.getPossibleGroups());
				// for our project
			}
			...
			if(CMInfo._CM_DEBUG)
			{
				...
				// for our project
				System.out.format("%-20s%-20s%-10d%-10d%-10d%n", tInfo.getSessionName(), tInfo.getAddress(), 
						tInfo.getPort(), tInfo.getUserNum(), tInfo.getPossibleGroups());
				// for our project
			}
		}
