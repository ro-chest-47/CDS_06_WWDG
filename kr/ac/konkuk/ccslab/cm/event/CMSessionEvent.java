...
public class CMSessionEvent extends CMEvent {
	...
	public boolean addGroupInfo(CMGroupInfo gi)
	{
		...
		// gi를 검색. 검색 후 없으면 (tgi == null)이면 그때 추가.
	...
	protected int getByteNum()
	{
		...
		case RESPONSE_SESSION_INFO:
			...
			while(iterSessionList.hasNext())
			{
				...
				nElementByteNum += 3*Integer.BYTES;
				// for our project.
		...
		case JOIN_SESSION_ACK:
			...
			// 어디서부터 m_groupList가 만들어졌길래 바로 iterator가 존재하는것..?
	...
	protected void marshallBody()
	{
		...
		case RESPONSE_SESSION_INFO:
			...
			while(iterSessionList.hasNext())
			{
				...
				// for our project
				m_bytes.putInt(tsi.getPossibleGroups());
				// for our project
			}
			break;
		case JOIN_SESSION:
		...
		case JOIN_SESSION_ACK:
			...
			while(iterGroupList.hasNext())
			{
				...
				if (tgi.getGroupState())
					m_bytes.putInt(0);
				else
					m_bytes.putInt(-1);
				m_bytes.putInt(tgi.getPossibleUser());
	...
	protected void unmarshallBody(ByteBuffer msg)
	{
		...
		case RESPONSE_SESSION_INFO:
			...
			for(i = 0; i < m_nSessionNum; i++)
			{
				...
				// for our project
				tsi.setPossibleGroups(msg.getInt());
				// for our project
		...
		case JOIN_SESSION_ACK:
			...
			for(i = 0; i < m_nGroupNum; i++)
			{
				...
				// for our project
				if (msg.getInt() == 0)
					tgi.setGroupStateTrue();
				else
					tgi.setGroupStateFalse();
				tgi.setPossibleUser(msg.getInt());
				// for our project
