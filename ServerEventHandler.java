...
public class ServerEventHandler implements CMAppEventHandler {
	...
	private void dummyEvent(CMEvent cme) {
		...
		int maxSession = s_stub.getCMInfo().getConfigurationInfo().getSessionNumber()+1;
		int maxGroup = 0;
		...
		case "0010":
			...
			if (session != null) {
				...
				maxGroup = session.getGroupList().size() + 1;
				...
				if (group != null) {
					...
					reply.setHandlerSession("null");
					reply.setHandlerGroup("null");
					reply.setDummyInfo("0011&SERVER&YES");
					
					s_stub.send(reply, token[1]);
				}
				else {
					System.out.println("Error 0010-1. group is null");
				}
			}
			else {
				System.out.println("Error 0010. session is null");
			}
				...
				if (group != null) {
					...
					for (int i = 0; i < tUser.size(); i++) {
						...
						if (tUser.get(i).getName().contentEquals(token[1])) {
							target = i;
							if (!tUser.get(target).getIsPlay() || // false : before playing, user exit.
									(tUser.get(target).getIsPlay() && groupUser == 5)) // true but groupUser == 5, the last user out.	
								group.setGroupStateTrue();
					...
					System.out.println("error 0020-1. group is null.");
				}
			}
			else {
				System.out.println("error 0020. session is null.");
			}
			break;
		...
		case "0030":
			...
			if (session != null) {
				...
				System.out.println("error 0030. session is null.");
			}
		...
		case "0040":
			...
			if(session != null) {
				...
				System.out.println("error 0040. session is null.");
			}
			...
			// user들의 정보는 ServerUser class에 별도로 저장되어 있음.
		case "0050":
			...
			if (session != null) {
				...
				System.out.println("error 0050. session is null.");
			}
			...
			reply.setDummyInfo("0051&SERVER&"+state);
			s_stub.send(reply, token[1]);
